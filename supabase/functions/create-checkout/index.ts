...
function validateTimestamp(ts: string | null): boolean {
  if (!ts) return true; // Don't block if not present (backwards compat)
...
function isSuspiciousUserAgent(ua: string): boolean {
  const suspicious = ['burp', 'postman', 'insomnia', 'curl', 'wget', 'httpie', 'python-requests'];
...
serve(async (req) => {
  ...
  try {
    ...
    logStep("Received request", { priceId, email: email?.substring(0, 5) + "***" });
