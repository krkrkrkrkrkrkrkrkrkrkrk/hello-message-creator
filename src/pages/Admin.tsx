...
interface CryptoPayment {
  ...
  status: string;
  created_at: string;
  manual_approval_requested: boolean;
...
const Admin = () => {
  ...
  const checkAdminAndLoad = async () => {
    try {
      ...
      setIsAdmin(true);
      await Promise.all([loadPromoCodes(), loadSales(), loadSalesStats(), loadCryptoPayments()]);
    } catch (error) {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/admin-manage-user`,
        {
          ...
          body: JSON.stringify({ action: 'get_crypto_payments' }),
        }
      );
  ...
  const approveCryptoPayment = async (paymentId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/admin-manage-user`,
        {
          ...
          body: JSON.stringify({ action: 'approve_crypto_payment', paymentId }),
        }
      );
      ...
      toast.success("Pagamento aprovado! API Key gerada.");
  ...
  const rejectCryptoPayment = async (paymentId: string) => {
    setApprovingPayment(paymentId);
    try {
      const { data: { session } } = await supabase.auth.getSession();
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/admin-manage-user`,
        {
          ...
          body: JSON.stringify({ action: 'reject_crypto_payment', paymentId }),
        }
      );
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative z-10 container max-w-6xl mx-auto px-4 py-8">
        ...
        <Tabs value={activeTab} onValueChange={(v) => {
          ...
          if (v === "crypto" && cryptoPayments.length === 0) loadCryptoPayments();
        }}>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">
                    {cryptoPayments.filter(p => p.status === 'pending').length} pagamentos pendentes
                  </span>
                </div>
                <Button variant="outline" onClick={loadCryptoPayments} disabled={cryptoLoading} className="gap-2">
              ...
              <div className="glass-card rounded-2xl border border-border/50 overflow-hidden">
                <div className="p-4 border-b border-border/50">
                  <h2 className="font-semibold flex items-center gap-2">
                    ...
                    Pagamentos Crypto ({cryptoPayments.length})
                  </h2>
                </div>
                ...
                {cryptoLoading ? (
                  ...
                  <div className="p-8 text-center text-muted-foreground">
                    ...
                    <p>Nenhum pagamento crypto encontrado</p>
                  </div>
                ) : (
                  <div className="divide-y divide-border/50 max-h-[600px] overflow-y-auto">
                    {cryptoPayments.map((payment, index) => (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
                            payment.status === 'completed' ? 'bg-primary/10' :
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <span className={`text-xs px-2 py-0.5 rounded-full ${
                                payment.status === 'completed' ? 'bg-primary/20 text-primary' :
                                payment.status === 'rejected' ? 'bg-destructive/20 text-destructive' :
                                payment.status === 'confirming' ? 'bg-blue-500/20 text-blue-500' :
                            ...
                            <div className="flex items-center gap-3 text-sm text-muted-foreground mt-1">
                              ...
                              <span>•</span>
                              <span>{payment.duration_days} dias</span>
                              <span>•</span>
                              <span>{new Date(payment.created_at).toLocaleString()}</span>
                            </div>
                            {payment.api_key && (
                              <div className="mt-1">
                                <code className="text-xs bg-muted px-2 py-1 rounded">{payment.api_key}</code>
                              </div>
                            )}
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          <div className="text-right mr-4">
                            <div className="font-bold text-lg text-primary">${Number(payment.amount).toFixed(2)}</div>
                          </div>
                          {payment.status === 'pending' && (
                            <div className="flex gap-2">
                              <Button
                                size="sm"
                                onClick={() => approveCryptoPayment(payment.id)}
                              ...
                              <Button
                                size="sm"
                                variant="destructive"
                                onClick={() => rejectCryptoPayment(payment.id)}
                                disabled={approvingPayment === payment.id}
                                className="gap-1"
                              >
